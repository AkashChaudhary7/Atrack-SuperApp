  ...
  return (
    <div className="space-y-5 font-sans">
      ...
      <div className="grid grid-cols-2 gap-3 z-10 relative">
        ...
        {/* Bento Card 2: Safe-To-Spend Budget Ledger */}
        <motion.div 
          whileHover={{ scale: 1.015, y: -2 }}
          transition={{ type: "spring", stiffness: 350, damping: 25 }}
          onClick={() => setActiveTab("expenses")}
          className={`border rounded-2xl p-3 flex flex-col justify-between cursor-pointer group shadow-3xs transition-all h-[105px] text-left ${
            isCircadian 
              ? "bg-[#0c1612] border-emerald-900/35 hover:border-emerald-600/50" 
              : "bg-white border-slate-150 hover:border-indigo-400/40"
          }`}
        >
          <div className="flex items-center justify-between">
            <div className={`p-1.5 rounded-lg ${isCircadian ? "bg-[#11241a] text-emerald-450" : "bg-indigo-50 text-indigo-600"}`}>
              <Wallet className="w-4 h-4" />
            </div>
            <span className={`text-[8px] font-mono tracking-wider font-bold uppercase py-0.5 px-1.5 rounded-md ${
              isCircadian 
          ...
          <div>
            <span className="text-[8.5px] text-slate-400 font-bold uppercase tracking-wider block font-mono">Daily Allowance</span>
            <span className={`text-base font-black font-mono leading-none ${isCircadian ? "text-emerald-300" : "text-slate-800"}`}>
              ₹{safeToSpendToday}
            </span>
        ...
        >
          <div className="flex items-center justify-between">
            <div className={`p-1.5 rounded-lg ${isCircadian ? "bg-amber-950/30 text-amber-400" : "bg-amber-50 text-amber-600"}`}>
              <ClipboardList className="w-4 h-4" />
            </div>
            <span className={`text-[8px] font-mono tracking-wider font-bold uppercase py-0.5 px-1.5 rounded-md ${
              isCircadian 
                ? "bg-amber-950/20 border border-amber-900/20 text-amber-400" 
          ...
          <div>
            <span className="text-[8.5px] text-slate-400 font-bold uppercase tracking-wider block font-mono">Completed Today</span>
            <span className={`text-base font-black font-mono leading-none ${isCircadian ? "text-emerald-300" : "text-slate-800"}`}>
              {habitCompletionPercent}%
            </span>
            <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-1 mt-1.5 overflow-hidden">
              <div 
                className={`h-full rounded-full ${isCircadian ? "bg-emerald-400" : "bg-indigo-600"}`} 
                style={{ width: `${habitCompletionPercent}%` }} 
              />
            </div>
            <span className="text-[8px] text-slate-400 block mt-1 font-bold">
              {habitsCompletedToday}/{totalHabitsCount} Achieved
            </span>
          </div>
        </motion.div>

        {/* Bento Card 4: Focus & Study Stamina */}
        <motion.div 
          whileHover={{ scale: 1.015, y: -2 }}
          transition={{ type: "spring", stiffness: 350, damping: 25 }}
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <span className={`text-[8px] font-mono tracking-wider font-bold uppercase py-0.5 px-1.5 rounded-md ${
              isCircadian 
                ? "bg-emerald-950/40 border border-emerald-900/20 text-emerald-400" 
          ...
          <div>
            <span className="text-[8.5px] text-slate-400 font-bold uppercase tracking-wider block font-mono">Sprint Total</span>
            <span className={`text-base font-black font-mono leading-none ${isCircadian ? "text-emerald-300" : "text-slate-800"}`}>
              {totalStudyMinutes} <span className="text-[10px] font-semibold text-slate-450">min</span>
            </span>
            <span className="text-[8px] text-slate-400 block mt-1.5 font-bold">
              {studySessions.length} Focus Blocks
            </span>
          </div>
        </motion.div>

        {/* Bento Card 5: Physical Wellness Reboot */}
        <motion.div 
          whileHover={{ scale: 1.015, y: -2 }}
          transition={{ type: "spring", stiffness: 350, damping: 25 }}
        ...
        >
          <div className="flex items-center justify-between">
            <div className={`p-1.5 rounded-lg ${isCircadian ? "bg-rose-950/40 text-rose-450" : "bg-rose-50 text-rose-600"}`}>
          ...
          <div>
            <span className="text-[8.5px] text-slate-400 font-bold uppercase tracking-wider block font-mono">Semen Retention</span>
            <span className={`text-base font-black font-mono leading-none ${isCircadian ? "text-emerald-300 font-extrabold" : "text-slate-800"}`}>
              {currentRetentionStreak} <span className="text-[10px] font-black text-rose-505">Days</span>
            </span>
            <span className="text-[8px] text-slate-400 block mt-1.5 font-bold">
              Clean Day Cycle Lock
            </span>
          </div>
        </motion.div>
      </div>
