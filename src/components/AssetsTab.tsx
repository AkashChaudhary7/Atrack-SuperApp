...
export const AssetsTab: React.FC = () => {
  const { assetAccounts, addAssetAccount, deleteAssetAccount, updateAssetBalance, expenses, isCircadian } = useApp();
  ...
  // Projected compounding parameters
  ...
  // Future valuation projections for wealth accumulation (Projected Net Worth Engine)
  const calculateCompoundingNetWorth = (years: number) => {
    const P = totalAssets;
    const PMT = parseFloat(monthlyContribution) || 0;
    const r = (parseFloat(roiRate) || 0) / 100;
    const n = years * 12;
    ...
    const principalTerm = P * Math.pow(1 + monthlyRate, n);
    const contributionsTerm = PMT * ((Math.pow(1 + monthlyRate, n) - 1) / monthlyRate);
    return principalTerm + contributionsTerm;
  };

  const projected1Yr = calculateCompoundingNetWorth(1);
  const projected5Yr = calculateCompoundingNetWorth(5);
  const projected10Yr = calculateCompoundingNetWorth(10);
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      {/* PROJECTED WEALTH ENGINE SECTION (COMPACT PRE-ESTIMATOR) */}
      <div className={`border p-4.5 rounded-2xl space-y-4 shadow-sm text-left transition-colors duration-300 ${
        isCircadian 
          ? "bg-[#0e1411] border-emerald-950 text-emerald-100" 
        ...
        <div>
          <div className="flex items-center gap-1.5 text-xs font-black uppercase tracking-wider font-mono">
            ...
            <span>Projected Net Worth Engine</span>
          </div>
          <p className="text-[10px] text-slate-400 font-semibold mt-0.5">
            Compound value forecasts using current net worth base (₹{totalAssets.toLocaleString("en-IN")})
          </p>
        </div>

        {/* Compound Controls Input forms */}
        <div className="grid grid-cols-2 gap-3.5">
          <div>
            <label className="text-[9px] font-bold block mb-1 uppercase tracking-wider font-mono text-slate-400">Monthly Contribution</label>
            <div className="relative">
              <span className="absolute inset-y-0 left-0 pl-2.5 flex items-center text-[10px] text-slate-450 font-bold font-sans">₹</span>
              <input 
                type="number" 
                value={monthlyContribution}
                onChange={(e) => setMonthlyContribution(e.target.value)}
                placeholder="0"
                className={`w-full text-xs font-bold font-mono pl-6 pr-2 py-1.5 rounded-xl border focus:outline-none ${
                  isCircadian 
                    ? "bg-[#070b09] border-emerald-900 text-emerald-350 focus:border-emerald-600" 
          ...
          <div>
            <label className="text-[9px] font-bold block mb-1 uppercase tracking-wider font-mono text-slate-400">Expected ROI (% p.a.)</label>
            <div className="relative">
              <input 
                type="number" 
                step="0.5"
                ...
                placeholder="10"
                className={`w-full text-xs font-bold font-mono px-3 py-1.5 rounded-xl border focus:outline-none pr-6 ${
                  isCircadian 
                    ? "bg-[#070b09] border-emerald-900 text-emerald-350 focus:border-emerald-600" 
                    : "bg-white border-slate-200 text-slate-800 focus:border-indigo-400"
                }`}
              />
              <span className="absolute inset-y-0 right-0 pr-2.5 flex items-center text-[10px] text-slate-450 font-mono font-bold">%</span>
            </div>
          </div>
        </div>

        {/* Compounded Results Grid of Micro Bars */}
        <div className="space-y-3.5 pt-2 border-t border-slate-200/40 dark:border-emerald-950/40">
          
          {/* Result 1: 1 Year Projection */}
          <div className="space-y-1">
            <div className="flex justify-between items-center text-[10px] font-mono">
              <span className="font-bold text-slate-450">1-YEAR HORIZON</span>
              <span className={`font-black ${isCircadian ? "text-emerald-400" : "text-slate-800"}`}>
                ₹{projected1Yr.toLocaleString("en-IN", { maximumFractionDigits: 0 })}
              </span>
            </div>
            <div className="w-full bg-slate-200/40 dark:bg-[#070b09] rounded-full h-1.5 overflow-hidden">
              <div 
                className={`h-full rounded-full transition-all duration-300 ${isCircadian ? "bg-emerald-500/60" : "bg-indigo-500/80"}`} 
                style={{ width: "30%" }} 
              />
            </div>
          </div>

          {/* Result 2: 5 Year Projection */}
          <div className="space-y-1">
            <div className="flex justify-between items-center text-[10px] font-mono">
              <span className="font-bold text-slate-450">5-YEAR ACCUMULATION</span>
              <span className={`font-black ${isCircadian ? "text-emerald-350 font-extrabold" : "text-slate-800"}`}>
                ₹{projected5Yr.toLocaleString("en-IN", { maximumFractionDigits: 0 })}
              </span>
            </div>
            <div className="w-full bg-slate-200/40 dark:bg-[#070b09] rounded-full h-1.5 overflow-hidden">
              <div 
                className={`h-full rounded-full transition-all duration-300 ${isCircadian ? "bg-emerald-400" : "bg-indigo-600"}`} 
                style={{ width: "65%" }} 
              />
            </div>
          </div>

          {/* Result 3: 10 Year Projection */}
          <div className="space-y-1">
            <div className="flex justify-between items-center text-[10px] font-mono">
              <span className="font-bold text-slate-455">10-YEAR COMPOUND MAXIMUM</span>
              <span className={`font-black ${isCircadian ? "text-emerald-300 font-extrabold" : "text-indigo-700"}`}>
                ₹{projected10Yr.toLocaleString("en-IN", { maximumFractionDigits: 0 })}
              </span>
            </div>
            <div className="w-full bg-slate-200/40 dark:bg-[#070b09] rounded-full h-1.5 overflow-hidden">
              <div 
                className={`h-full rounded-full transition-all duration-300 ${isCircadian ? "bg-emerald-300" : "bg-indigo-700"}`} 
                style={{ width: "100%" }} 
              />
            </div>
          </div>
