...
export const HabitsTab: React.FC = () => {
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      {/* Test reminder card */}
      <div className={`border rounded-2xl p-3 flex flex-col gap-2.5 shadow-sm text-xs transition-colors duration-300 ${isCircadian ? "bg-emerald-950/20 border-emerald-900/45 text-emerald-150" : "bg-amber-50/50 border-amber-250 text-slate-700"}`}>
        <div className="flex items-center justify-between">
          <div>
            <h4 className={`font-bold ${isCircadian ? "text-emerald-400" : "text-amber-800"}`}>Habit Alert Verification</h4>
            <p className={`text-[10px] mt-0.5 ${isCircadian ? "text-emerald-500/80" : "text-slate-500"}`}>Tests the Web Notification system (by simulating 8:00 PM status)</p>
          </div>
          <button
            type="button"
            onClick={() => {
                  ...
                  if (permission === "granted") {
                    ...
                    if (incomplete.length > 0) {
                      try {
                        ...
                        console.error("Failed executing simulate alert constructor:", err);
                      }
                    } else {
                      try {
                        new Notification("Habit Reminder (Simulated)", {
                        ...
                        console.error("Failed executing simulate success constructor:", err);
                      }
                    }
                  } else {
                    setNotifError("Notification permission was denied or is blocked by browser policies inside this iframe context.");
                  }
                }).catch(err => {
                  setNotifError("Notification request is restricted inside this sandboxed preview iframe context.");
                });
              } else {
                setNotifError("Web Notifications are not supported in your browser.");
              }
            }}
            className={`font-bold px-2.5 py-1.5 rounded-xl transition-all border text-[10px] cursor-pointer ${
              isCircadian 
                ? "bg-emerald-900/40 hover:bg-emerald-900/60 text-emerald-400 border-emerald-805" 
        ...
        {notifError && (
          <div className={`text-[10px] p-2.5 rounded-xl leading-normal mt-0.5 flex flex-col gap-1.5 border font-medium ${
            isCircadian 
              ? "bg-[#1f1012] border-rose-950 text-rose-350" 
              : "bg-rose-50/75 border-rose-150 text-rose-700"
          }`}>
            <span>⚠️ **Alert blocked:** {notifError}</span>
            <hr className={`border-[0.5px] ${isCircadian ? "border-rose-950/60" : "border-rose-150"}`} />
            <div className="flex items-center gap-1.5">
              ...
              <a 
                ...
                className={`font-black uppercase tracking-wider text-[9px] hover:underline ${
                  isCircadian ? "text-emerald-400" : "text-indigo-600"
                }`}
              >
                Open in standalone tab to clear permission blocks ↗
              </a>
            </div>
          </div>
        )}
